...
func run(cmd *cobra.Command, _ []string, opts *Options) error {
	...
	if err := addTagToVGs(cmd.Context(), setupClient, lvm, nodeName, namespace, opts.SetupLog); err != nil {
		return err
	}
...
func addTagToVGs(ctx context.Context, c client.Client, lvm *lvm.HostLVM, nodeName string, namespace string, logger logr.Logger) error {
	vgs, err := lvm.ListVGs()
	if err != nil {
